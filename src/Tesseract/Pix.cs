...
namespace Tesseract
{
	public unsafe sealed class Pix: DisposableBase, IEquatable<Pix>
	{
		...
		{
			if (handle == IntPtr.Zero) throw new ArgumentException("Pix handle must not be zero (null).",nameof(handle));
		...
		public Pix BinarizeOtsuAdaptiveThreshold(int sx,int sy,int smoothx,int smoothy,float scorefract)
		{
			...
			IntPtr ppixth, ppixd;
			var result = Interop.LeptonicaApiSignatures.pixOtsuAdaptiveThreshold(handle,sx,sy,smoothx,smoothy,scorefract,out ppixth,out ppixd);
		...
		public Pix BinarizeSauvola(int whsize,float factor,bool addborder)
		{
			...
			var maxWhSize = System.Math.Min((Width - 3) / 2,(Height - 3) / 2);
			...
			var result = Interop.LeptonicaApiSignatures.pixSauvolaBinarize(handle,whsize,factor,addborder ? 1 : 0,out ppixm,out ppixsd,out ppixth,out ppixd);
		...
		public Pix BinarizeSauvolaTiled(int whsize,float factor,int nx,int ny)
		{
			...
			IntPtr ppixth, ppixd;
			var result = Interop.LeptonicaApiSignatures.pixSauvolaBinarizeTiled(handle,whsize,factor,nx,ny,out ppixth,out ppixd);
		...
		public Pix RemoveLines()
		{
			...
			try
			{
				...
				pix2 = Interop.LeptonicaApiSignatures.pixRotateAMGray(handle,(float)(Deg2Rad * angle),255);
		...
		public Pix Rotate(float angleInRadians,RotationMethod method = RotationMethod.AreaMap,RotationFill fillColor = RotationFill.White,int? width = null,int? height = null)
		{
			...
			var rotations = 2 * angleInRadians / System.Math.PI;
		...
		{
			var resultHandle = Interop.LeptonicaApiSignatures.pixRotate90(handle,direction);
		...
		{
			var result = Interop.LeptonicaApiSignatures.pixScale(handle,scaleX,scaleY);
