...
namespace Tesseract.Tests
{
	...
	public class ResultRendererTests: TesseractTestBase
	{
		...
		{
			var resultPath = TestResultRunFile(@"ResultRenderers\PDF\phototest");
			var formats = new List<RenderedFormat> { RenderedFormat.HOCR,RenderedFormat.PDF,RenderedFormat.TEXT };
		...
		public void CanRenderMultiplePageDocumentIntoMultipleResultRenderers()
		{
			...
			Assert.IsTrue(File.Exists(expectedPdfOutputFilename),$"Expected a PDF file \"{expectedPdfOutputFilename}\" to have been created; but none was found.");

			var expectedTxtOutputFilename = Path.ChangeExtension(resultPath,"txt");
			Assert.IsTrue(File.Exists(expectedTxtOutputFilename),$"Expected a Text file \"{expectedTxtOutputFilename}\" to have been created; but none was found.");
		}
